...
import { Dialog, DialogTitle, DialogContent, TextField, DialogActions, Button, FormControl, InputLabel, Select, MenuItem, List, ListItem, Box, ListItemText, IconButton, ListItemAvatar, Avatar } from "@mui/material";
...
const requiredText = "Trường này là bắt buộc!"
const maximum50Character = "Vui lòng nhập tối đa 50 ký tự"
const maximum255Character = "Vui lòng nhập tối đa 255 ký tự"
const moreThanZero = "Vui lòng nhập số lớn hơn 0"

const ProductDialog: React.FC<ProductDialogProps> = ({ open, setOpen, type, product, setReload, reload }) => {
    ...
    const updateProduct = (product: IProduct) => {
        axios.put(`${ApiPathEnum.Product}/${product?._id}`, product).then(res => {
            if (res.status === 200) {
                setReload(!reload)
            }
        })
    ...
    const formik = useFormik({
        initialValues: {
            ...
            categoryId: type === 'CREATE' ? '' : (product?.categoryId as ICategory)?._id
        } as IProduct,
        onSubmit,
        ...
        if (evt.target.files) {
            for (const file of evt.target.files) {
                ...
                axios
                    ...
                    .then(res => {
                        if (res.status === 201) {
                            const images = res.data.data.map((x: any) => ({ id: x.public_id, url: x.url }))
    ...
    useEffect(() => {
        if (open) {
            ...
            if (product) {
                const adjustProduct = { ...product, categoryId: (product.categoryId as ICategory)?._id as string }
    ...
    >
        ...
        <form onSubmit={formik.handleSubmit}>
            <DialogContent>
                ...
                >
                    Tải ảnh sản phẩm
                    <VisuallyHiddenInput type="file" multiple={true} onChange={handleUploadImage} />
                </Button>
                {/*                 
                <TextField
                    sx={{ my: 1 }}
                ...
                <List >
                    {
                        formik.values.images.map(x =>
                            <ListItem key={x.id} sx={{ gap: 2 }}
                                secondaryAction={
                                    <IconButton edge="end" aria-label="delete" onClick={() => { deleteImage(x.id) }}>
                                        <DeleteIcon />
                                    </IconButton>
                                }>
                                <ListItemAvatar>
                                    <Avatar sx={{ width: 100, height: 100 }}>
                                        <Box component='img' src={x.url} width={1} height={1} />
                                    </Avatar>
                                </ListItemAvatar>
                                <ListItemText
                                    sx={{ overflow: 'hidden' }}
